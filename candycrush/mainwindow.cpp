...
{
    link[i][j]=true;
    if(!link[i-1][j] && i-1>=0 && colorgroup(map[i][j])==colorgroup(map[i-1][j]) && colorgroup(map[i][j]).i!=5)
        if((i-2>=0 && colorgroup(map[i][j])==colorgroup(map[i-2][j])) || (i+1<10 && colorgroup(map[i][j])==colorgroup(map[i+1][j])))
        {
            check=true;
            DFS(i-1, j);
        }
    if(!link[i][j-1] && j-1>=0 && colorgroup(map[i][j])==colorgroup(map[i][j-1]) && colorgroup(map[i][j]).i!=5)
        if((j-2>=0 && colorgroup(map[i][j])==colorgroup(map[i][j-2])) || (j+1<10 && colorgroup(map[i][j])==colorgroup(map[i][j+1])))
        {
            check=true;
            DFS(i, j-1);
        }
    if(!link[i+1][j] && i+1<10 && colorgroup(map[i][j])==colorgroup(map[i+1][j]) && colorgroup(map[i][j]).i!=5)
        if((i-1>=0 && colorgroup(map[i][j])==colorgroup(map[i-1][j])) || (i+2<10 && colorgroup(map[i][j])==colorgroup(map[i+2][j])))
        {
            check=true;
            DFS(i+1, j);
        }
    if(!link[i][j+1] && j+1<10 && colorgroup(map[i][j])==colorgroup(map[i][j+1]) && colorgroup(map[i][j]).i!=5)
